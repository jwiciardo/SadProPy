...
class NonlinearTimeHistoryAnalysis:
    ...
    def _perform_NonlinearTimeHistoryAnalysis(self, N_acc, dt, tol=1.0e-6, Maxiter=5000, nSteps=1, pFlag=1):
        """
        Input
        -----
        tol : float
        ...
        ops.test('NormDispIncr', tol, Maxiter, pFlag) # Setting when convergence has been achieved
                    # algoType
        ops.algorithm('Newton') # Setting the sequence of steps taken to solve the non-linear equation at the current time step
        # numbererType:'RCM'
        ...
        ops.constraints('Transformation') # Setting how the constraint equations are enforced in the analysis
                       # intType, gamma, beta
        ops.integrator('Newmark', 0.5,   0.25) # Setting the predictive step for time (t+dt)
        # analysisType: 'Transient'
        ops.analysis('Transient') # Creating Analysis objects
            
        # PERFORM ANALYSIS
                # numIncr
        ok = ops.analyze(4129, dt) # Performing the Analysis
        print(ok)
