...
class ModelDataStorer:
    ...
    def retrieve(self):
        ...
        return ModelData(
            ...
            mat_concrete04 = self._translator_data["Mat: Concrete04"],
            mat_steel02 = self._translator_data["Mat: Steel02"],
            mat_minmax = self._translator_data["Mat: Minmax"],
            mat_imk = self._translator_data["Mat: IMK"],
            materials_list = self._translator_data["Materials List"],
            frame_sections = self._translator_data["Frame Sections"],
            sec_fiber = self._translator_data["Sec: Fiber"],
            sec_aggregator = self._translator_data["Sec: Aggregator"],
            sections_list = self._translator_data["Sections List"],
    ...
    def _generate_elements(self, nodes):
        ...
        sec_class = line_objects["Section Class"][mask]
        ...
        autogen_offsets_length = autogenerate_offsets_length(
            secs_list=self._translator_data["Sections List"],
            sec_class=sec_class,
            sec_idx=sec_idx,
