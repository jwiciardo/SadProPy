...
def _plot_elemental_loads(ax, units, coords, elements, distributed_loads, concentrated_loads, loadcase, loadcase_type, scale, show_labels, is_arrow=True, linewidth=0.8):
    ...
    if len(concentrated_loads.element_tag) == 0 and len(distributed_loads.element_tag) == 0:
        warningtext(
            "No elemental loads were defined"
        )
    ...
    def plot_concentrated_loads():
        ...
        default_load_magnitude = to_internalunits.concentrated_lineload(values=75)
        normalised_load_magnitude = (np.divide(load_magnitude, default_load_magnitude, out=np.zeros_like(load_magnitude), where=default_load_magnitude > 0.0)).ravel() # Determine normalised load magnitude
        arrow_length = scale * normalised_load_magnitude # Determine scalable arrow length
        global_vector_arrow = global_vector_loads * arrow_length[:, None] # Determine vector of arrow in global axes
        arrow_tip_coords = arrow_coords + (-1 * global_vector_arrow)

        # Plot load arrow
        ax.quiver(
            arrow_coords[:, 0], arrow_coords[:, 1], arrow_coords[:, 2],
            global_vector_arrow[:, 0], global_vector_arrow[:, 1], global_vector_arrow[:, 2],
            color="tab:red",
            linewidth=linewidth,
            pivot="tip",
            zorder=5,
        ) # Plot arrow for load direction

        # Plot load magnitude value
        visualised_load_magnitude = from_internalunits.concentrated_lineload(load_magnitude.ravel()) # Convert load units
        model_size = np.max(coords.max(axis=0) - coords.min(axis=0))
        offset = 0.005 * model_size * (-1 * global_vector_loads) # Set label offset
        if show_labels: # Set condition if show_labels is True or False
            for i in range(n):
                ax.text(
                    arrow_tip_coords[i, 0] + offset[i, 0],
                    arrow_tip_coords[i, 1] + offset[i, 1],
                    arrow_tip_coords[i, 2] + offset[i, 2],
    ...
    def plot_distributed_loads():
        ...
        global_vector_loads = transform_to_global_axes(values=vector_loads, rotation_matrices=rotation_matrices) # Determine unit vector of loads in global axes
        default_load_magnitude = to_internalunits.distributed_lineload(values=7.5)
        normalised_load_magnitude = np.divide(load_magnitude, default_load_magnitude, out=np.zeros_like(load_magnitude), where=default_load_magnitude > 0.0) # Determine normalised load magnitude
        arrow_length = scale * normalised_load_magnitude # Determine scalable arrow length
        global_vector_arrow = global_vector_loads * arrow_length[:, :, None] # Determine vector of arrow in global axes
        ...
        ax.add_collection3d(Poly3DCollection(
            polygon_vertices,
            facecolor="None" if is_arrow else "tab:blue",
            edgecolor="tab:blue",
            alpha=0.20,
        ...
        # Plot load magnitude value
        visualised_load_magnitude_start = from_internalunits.distributed_lineload(load_magnitude[row_idx, 0]) # Convert load units
        visualised_load_magnitude_end = from_internalunits.distributed_lineload(load_magnitude[row_idx, last_idx]) # Convert load units
        model_size = np.max(coords.max(axis=0) - coords.min(axis=0))
        offset = 0.005 * model_size * (-1 * global_vector_loads) # Set label offset
        if show_labels: # Set condition if show_labels is True or False
            for i in range(n):
                ax.text(
                    arrow_tip_coords[i, 0, 0] + offset[i, 0, 0],
                    arrow_tip_coords[i, 0, 1] + offset[i, 0, 1],
                    arrow_tip_coords[i, 0, 2] + offset[i, 0, 2],
                ...
                ax.text(
                    arrow_tip_coords[i, last_idx[i], 0] + offset[i, last_idx[i], 0],
                    arrow_tip_coords[i, last_idx[i], 1] + offset[i, last_idx[i], 1],
                    arrow_tip_coords[i, last_idx[i], 2] + offset[i, last_idx[i], 2],
